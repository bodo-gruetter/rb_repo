...
To do this, we simply take our initial model, which we had formed using findings from exploratory graphical analysis. In addition, we take into account the interaction that we have already shown in the first more complex model. The following model results:
...
complex.model.2 <- price ~ yearOfRegistration + brand + fuelType + vehicleType +
s(powerPS) + gearbox * kilometer + age
...
All values of R-Squared are rather bad. The starting model closes worst, while the most complex model has the best value.
...
```{r, warning=FALSE}
star <- graph_from_literal(a:b:c--d--e:f:g)
...
gl <- graph_from_literal(a--b, c+-d, e-+f, g++h)
