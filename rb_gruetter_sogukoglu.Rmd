...
summary(gam.starting.model.1)$r.squared
...
Now that we have derived a relatively simple starting model, which considers only a single polynomial effect, we need a more complex model, which we can compare with the simple model. To get an appropriate comparison, we take our starting model, which we had defined before the model development, and add interactions to it. Thus, this model considers a new level of complexity on the one hand and more predictors on the other hand. The graphical analysis had shown a slight interaction effect between kilometer and gearbox. We would now like to take this into account. The following model results as comparison model:
